...
else 
{
	...
	if( isset($_GET['search']) )
	{
		...
		{
			$row = Database::get()->prepare_execute("SELECT * FROM event INNER JOIN tag_relation ON event.id = tag_relation.event_id INNER JOIN tag on tag.id = tag_relation.tag_id WHERE tag.name = '$name'" );
			if ( !empty($row) ) {
				$results = new View('View/display_listEvent.html');
				$results->setLoop(['LIST' => $row]);
				$message = 'Events you are looking for.';
