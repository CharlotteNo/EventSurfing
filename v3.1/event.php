...
else 
{
	...
	if( isset($_GET['id']) && is_numeric($_GET['id']) )
	{
		...
		if ( !empty($eventrow) )
		{
			...
			$tagrow = Database::get()->prepare_execute("SELECT name FROM tag INNER JOIN tag_relation ON tag_relation.id = tag.id WHERE event_id = '$id'" );
			if ( !empty($tagrow) ) {
				$taglist = new View('View/display_listTag.html');
				$taglist->setLoop(['LIST' => $tagrow]);
			}			
