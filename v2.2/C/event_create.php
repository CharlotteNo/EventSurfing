...
if( isset($_GET['create_event']) && !empty($_GET) )
{
	...
	if ( empty($errormessage) )
	{
		...
		Database::get()->insert_update("INSERT  INTO event (name, address, host, time, bio)
	            						VALUES ('$name', '$address','$id', '$time', '$bio')");

		//récupère l'id du nouvel évenement
		$req = Database::get()->prepare_execute("SELECT id FROM event WHERE name='$name' AND host='$id'"); //comment récuperer le bon id ? plusieurs conditions à vérifier ?
		$idEvent = $req[0]['id'];
		echo $idEvent;
		var_dump($idEvent);
